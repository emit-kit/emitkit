...
Real-time event streaming and notifications platform built with SvelteKit and Svelte 5.

## Features

- **Real-time Event Streaming**: SSE-based live updates with Tinybird (ClickHouse) backend
- **Multi-tenancy**: Organization → Site → Channel → Events
- **Push Notifications**: Web Push API with VAPID support
- **Webhooks**: HTTP POST dispatching with HMAC signatures
- **Analytics**: Integrated event tracking with Stacksee Analytics
