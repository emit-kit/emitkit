...
import { cleanupDeletedSites } from '$lib/features/sites/server/jobs/cleanup-deleted-sites';
import { createLogger } from '$lib/server/logger';

const logger = createLogger('cron-cleanup-sites');

/**
 * Cron endpoint to cleanup soft-deleted sites
 *
 * This endpoint should be called by a cron service (e.g., Vercel Cron, GitHub Actions)
 * to periodically clean up sites that have been soft-deleted beyond the retention period.
 *
 * Security:
 ...
 * {
 *   "crons": [{
 *     "path": "/api/cron/cleanup-sites",
 *     "schedule": "0 2 * * *"
 *   }]
 * }
 */
export const GET: RequestHandler = async ({ request }) => {
	const operation = logger.start('Cron job: cleanup sites');
	...
	try {
		...
		const result = await cleanupDeletedSites();
