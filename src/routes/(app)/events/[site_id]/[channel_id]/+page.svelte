<script lang="ts">
	...
	const query = await getEventsListQuery({
	...
	let loadedEventIds = new Set<string>(query ? query.items.map((event) => event.id) : []);

	// Initialize with server-loaded events (not marked as new)
	let allEvents = $state<EventWithNewStatus[]>(query ? query.items : []);
				...
				<div class="flex flex-col gap-3">
					...
					<div class="flex flex-col gap-3">
						{#each group.events as event (event.id)}
							<EventListItem {event} isNew={event.isNew ?? false} />
						{/each}
					</div>
				</div>
			{/each}
