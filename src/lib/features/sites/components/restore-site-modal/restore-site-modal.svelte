<script lang="ts">
	...
	import { restoreSiteCommand } from '$lib/features/sites/sites.remote';
	...
	async function handleRestore() {
		...
		try {
			...
			error = err instanceof Error ? err.message : 'Failed to restore site';
			isRestoring = false;
		}
	}
</script>
...
<Dialog.Root open={true}>
	<Dialog.Content class="sm:max-w-[500px]">
		<Dialog.Header>
			...
			<Dialog.Description>
				Restore this archived site and make it accessible again. All channels and API keys will be
				restored.
			</Dialog.Description>
		</Dialog.Header>
		...
		<div class="space-y-3 py-4">
			...
			<Alert.Root>
				...
				<Alert.Title>Restoring "{siteName}" will:</Alert.Title>
				<Alert.Description class="mt-2 space-y-2">
					<ul class="list-inside list-disc space-y-1 text-sm">
						<li><strong>Restore all channels</strong> associated with this site</li>
