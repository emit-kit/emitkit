		...
		subscriptions.map(async (subscription) => {
			try {
				...
				if (
					...
					(error.message.includes('410') || // Gone - subscription expired
						error.message.includes('404') || // Not found
						error.message.includes('InvalidSubscription'))
				) {
					...
					return { success: false, endpoint: subscription.endpoint, error: 'expired' };
				}
