<script lang="ts">
	...
	async function handleSubmit() {
		...
		try {
			...
			error = err instanceof Error ? err.message : 'Failed to connect Discord integration';
			isSubmitting = false;
		}
	}
</script>
...
<Dialog.Root open={true}>
	<Dialog.Content class="sm:max-w-[500px]">
		<Dialog.Header>
			<Dialog.Title>Connect Discord</Dialog.Title>
			<Dialog.Description>
				Configure Discord webhook to receive event notifications from Blip.
			</Dialog.Description>
		</Dialog.Header>
		...
		<div class="space-y-4">
			...
			<div class="space-y-2">
				...
				<p class="text-sm text-muted-foreground">
					Get your webhook URL from Discord's Server Settings > Integrations > Webhooks
				</p>
			</div>
