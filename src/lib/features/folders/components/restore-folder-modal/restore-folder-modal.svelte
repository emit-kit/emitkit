<script lang="ts">
	...
	import { restoreFolderCommand } from '$lib/features/folders/folders.remote';
	...
	async function handleRestore() {
		...
		try {
			...
			error = err instanceof Error ? err.message : 'Failed to restore folder';
			isRestoring = false;
		}
	}
</script>
...
<Dialog.Root open={true}>
	<Dialog.Content class="sm:max-w-[500px]">
		<Dialog.Header>
			...
			<Dialog.Description>
				Restore this archived folder and make it accessible again. All channels and API keys will be
				restored.
			</Dialog.Description>
		</Dialog.Header>
		...
		<div class="space-y-3 py-4">
			...
			<Alert.Root>
				...
				<Alert.Title>Restoring "{folderName}" will:</Alert.Title>
				<Alert.Description class="mt-2 space-y-2">
					<ul class="list-inside list-disc space-y-1 text-sm">
						<li><strong>Restore all channels</strong> associated with this folder</li>
		...
		<Dialog.Footer>
			...
			<Button type="button" onclick={handleRestore} disabled={isRestoring}>
				{isRestoring ? 'Restoring...' : 'Restore Folder'}
			</Button>
		</Dialog.Footer>
	</Dialog.Content>
</Dialog.Root>
